...
class ConstantTest {
  ...
  fun boolWithWrongTypeOfConstant() {
    ...
    shouldThrow<IllegalStateException> {
      Constant.validate(
          symbolTable, IdentifierValueElement(loc, "aBool", "aBool"), BuiltinType.BOOL)
    }
  }
  ...
  fun boolWithNonConstantIdentifier() {
    val e =
        shouldThrow<IllegalStateException> {
          Constant.validate(
              symbolTable,
              IdentifierValueElement(loc, "someStruct", "someStruct"),
              BuiltinType.BOOL)
        }

    e.message shouldContain
        "Expected 'true', 'false', '1', '0', or a bool constant; got: someStruct"
  }
  ...
  fun tooLargeInt() {
    val value =
        IntValueElement(loc, "${Integer.MAX_VALUE.toLong() + 1}", Integer.MAX_VALUE.toLong() + 1)
    val type = BuiltinType.I32

    val e = shouldThrow<IllegalStateException> { Constant.validate(symbolTable, value, type) }
  ...
  fun tooSmallInt() {
    val value =
        IntValueElement(loc, "${Integer.MIN_VALUE.toLong() - 1}", Integer.MIN_VALUE.toLong() - 1)
    val type = BuiltinType.I32
  ...
  fun doubleLiteral() {
    ...
    val e =
        shouldThrow<IllegalStateException> {
          Constant.validate(
              symbolTable, LiteralValueElement(loc, "aString", "aString"), BuiltinType.DOUBLE)
        }
  ...
  fun doubleConstant() {
    ...
    Constant.validate(
        symbolTable, IdentifierValueElement(loc, "aDouble", "aDouble"), BuiltinType.DOUBLE)
  }
  ...
  fun doubleWithWrongTypeOfConstant() {
    ...
    val e =
        shouldThrow<IllegalStateException> {
          Constant.validate(
              symbolTable, IdentifierValueElement(loc, "aString", "aString"), BuiltinType.DOUBLE)
        }
  ...
  fun enumWithMember() {
    ...
    Constant.validate(
        symbolTable, IdentifierValueElement(loc, "TestEnum.TEST", "TestEnum.TEST"), et)
  }
  ...
  fun enumWithNonMemberIdentifier() {
    ...
    val e =
        shouldThrow<IllegalStateException> {
          Constant.validate(
              symbolTable,
              IdentifierValueElement(loc, "TestEnum.NON_MEMBER", "TestEnum.NON_MEMBER"),
              et)
        }
    e.message shouldContain
        "'TestEnum.NON_MEMBER' is not a member of enum type TestEnum: members=[TEST]"
  }
  ...
  fun typedefOfWrongEnum() {
    ...
    val e =
        shouldThrow<IllegalStateException> { Constant.validate(symbolTable, value, typedefType) }
  ...
  private fun makeTypedef(oldType: ThriftType, newName: String): TypedefType {
    val element = TypedefElement(loc, ScalarTypeElement(loc, "does_not_matter", null), newName)

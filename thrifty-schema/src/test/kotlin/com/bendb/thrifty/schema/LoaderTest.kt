...
class LoaderTest {
  ...
  fun crazyIncludeReferencedConst() {
    ...
    referencedConstant.location.path shouldBe
        listOf("nested", "a.thrift").joinToString(File.separator)
  }
  ...
  fun addIncludeFileSmokeTest() {
    ...
    val schema = Loader().addIncludePath(tempDir.toPath()).load()
  ...
  fun invalidEnumStructValuedConst() {
    ...
    e.message shouldContain
        "'Membership.WRONG' is not a member of enum type Membership: members=[MEMBER, NON_MEMBER, UNDER_REVIEW]"
  }
  ...
  fun `struct const with reference to another const`() {
    ...
    val theContainer =
        consts.find { it.name == "THE_CONTAINER" }
            ?: error("Expected a constant named THE_CONTAINER")

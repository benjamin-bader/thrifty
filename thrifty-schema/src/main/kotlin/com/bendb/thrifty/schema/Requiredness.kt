...
/**
 * A description of whether and when a field may be absent from a struct, union, or exception.
 *
 * The semantics of required, optional, and the default are underspecified at present. Intuitively,
 * `required` should mean that the struct containing the field cannot be valid unless the field is
 * present, where `optional` would indicate that the field's presence does not affect the struct's
 * validity. The actual impacts of these modifiers are both more subtle and implementation-defined,
 * varying both by runtime library and code-generator implementations.
 *
 ...
 * underlying protocol. During validation, they determine whether an unset field will cause
 * validation to fail. As this is not documented anywhere, a close reading of the Apache
 ...
 * not "set" to be sent! In contrast, Thrifty treats default-requiredness "value types" as optional;
 * sending data that is not intentionally set is deeply suspect.
 */
enum class Requiredness {
  /**
   * No requirement. The semantics of this are undefined, but in practice can mean required for
   * serialization, optional for deserialization.
   *
  ...
  /**
   * Required. In practice this means that a field must be present both on serialization and on
   * deserialization.
   */
  REQUIRED,

...
/**
 * An object that can resolve the types of typdefs, struct fields, and service method parameters
 * based on types declared in Thrift [Program]s and their transitive included Programs.
 *
    ...
    for (p in program.includes) {
      ...
      if (ix == -1) {
        throw AssertionError(
            "No extension found for included file " +
  ...
  fun resolveType(type: TypeElement): ThriftType {
    ...
    return when (type) {
      ...
      is ScalarTypeElement -> {
        ...
        // we have an error.
        val builtinType = BuiltinType.get(type.name) ?: throw LinkFailureException(type.name)
      ...
      if (ix != -1) {
        ...
        constant =
            program.includes
                .asSequence()
                .filter { p ->
                  p.location.path == expectedPath
                } // TODO: Should this be ==, or endsWith?

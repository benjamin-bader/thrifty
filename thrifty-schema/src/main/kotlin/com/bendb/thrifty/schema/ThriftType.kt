 ...
 *
 * Nearly every top-level entity in the Thrift IDL is a type of some sort; only consts are not. A
 * type can be a built-in "primitive" like the numeric types, user-defined types like structs,
 * unions, exceptions, services, and typedefs, or containers like lists, sets, and maps.
 *
 * A ThriftType represents either the definition of a new type or a reference to an existing type;
 * the distinction after parsing is minimal, and surfaces primarily in what annotations are present.
 * For example, a `StructType` could define a struct, but it could also be the type of a field in
 * another struct. In both cases, it would contain the full definition, but the field's type might
 * carry additional annotations which would only be present on the instance held by that field.
 *
 ...
 * }`</pre>
 *
 * After parsing, we have (among others) three instances of `StructType`. One for `Bar`, and two for
 * `Foo`. The first instance is the initial definition of the struct; the second is that held by the
 * sole field of `Bar`. The first instance has a single annotation, `(label = "foo")`. The second
 * has *two* annotations: `(label = "foo", immutable = "true")`.
 *
 * @property name The name of this type.
 */
abstract class ThriftType internal constructor(open val name: String) {
  ...
  /**
   * True if this is a structured user-defined type such as a struct, union, or exception. This does
   * *not* mean that this type is actually a struct!
   *
  ...
  /**
   * Returns the aliased type if this is a typedef or, if not, returns `this`. A convenience
   * function for codegen.
   *
  ...
  /**
   * Accepts a [Visitor], performing an arbitrary operation and returning its result.
   *
   ...
   *
   * Annotations in the new map will overwrite duplicate names on this type; this instance is left
   * unmodified.
   *
   * @param annotations the annotations to add to the returned type.
   * @return a copy of `this` with the given `annotations`.
   */
   ...
   *
   * Just your standard visitor from the Gang of Four book. Very useful for code generation, which
   * is all about type-hierarchy-specific operations; visitors let us effectively augment a
   * complicated hierarchy without bloating the ThriftType classes themselves.
   *

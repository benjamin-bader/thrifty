...
/**
 * Represents a named value in a struct, union, or exception; a field also represents parameters
 * accepted by or exceptions thrown by service methods.
 */
class Field
private constructor(
   ...
   *
   * The difference is that redaction eliminates _all_ information, while obfuscation preserves
   * _some_. Typically, scalar values will be rendered as a hash code, and collections will be
   * rendered as a typename plus a size, e.g. `list<string>(size=12)`.
   */
  ...
  /**
   * If this field's type is a typedef, this value will be the name of the typedef itself - that is,
   * the "new" name, not the aliased type's name.
   */
  val typedefName: String?
    get() {
      return type_?.let { if (it.isTypedef) it.name else null }
    }
  ...
  class Builder internal constructor(field: Field) :
      AbstractUserElementBuilder<Field, Builder>(field.mixin) {
    ...
    fun type(type: ThriftType): Builder = apply { this.fieldType = type }

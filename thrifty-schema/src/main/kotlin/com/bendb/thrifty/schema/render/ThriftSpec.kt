...
internal constructor(
  ...
  /**
   * @return a render of this file. This returns valid thrift that could be parsed back and yield an
   *   equivalent [Schema] to this spec's [schema].
   */
   ...
   */
  fun <A : Appendable> renderTo(buffer: A, fileComment: String? = null) =
      buffer.apply {
        fileComment?.let {
          append("// ", it)
          appendLine()
        ...
        if (namespaces.isNotEmpty()) {
          namespaces.entries.joinEachTo(
              buffer = buffer, separator = NEWLINE, postfix = DOUBLE_NEWLINE) { _, (key, value) ->
                buffer.append("namespace ", key.thriftName, " ", value)
              }
        }
        if (includes.isNotEmpty()) {
          includes.sortedBy(Include::path).joinEachTo(buffer, NEWLINE, postfix = DOUBLE_NEWLINE) {
              _,
              include ->

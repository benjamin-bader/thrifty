...
/**
 * A mixin encapsulating a common implementation of [UserElement], which does not conveniently fit
...
internal data class UserElementMixin(
  ...
  /**
   * Checks for the presence of the given annotation name, in several possible varieties. Returns
   * true if:
   ...
   *
   * The latter two conditions are officially undocumented, but are present for legacy use. This
  ...
  internal class Builder internal constructor(userElement: UserElement) {
    ...
    fun location(location: Location): Builder = apply { this.location = location }

    fun documentation(documentation: String): Builder = apply {
      this.documentation =
          if (isNonEmptyJavadoc(documentation)) {
            documentation
          } else {
            ""
          }

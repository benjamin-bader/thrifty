...
class ServiceType : UserType {
  ...
  /**
   * The type of the service that this service extends, or null if this does not extend any other
   * service.
   */
    ...
    for (method in methods) {
      ...
      if (conflictingMethod != null) {
        ...
        linker.addError(
            ...
            "Duplicate method; '" +
                method.name +
                "' conflicts with another method declared at " +
                conflictingMethod.location)
      }
    }
  ...
  class Builder internal constructor(type: ServiceType) :
      UserType.UserTypeBuilder<ServiceType, Builder>(type) {
    ...
    fun methods(methods: List<ServiceMethod>): Builder = apply { this.methods = methods }

    /** Use the given [base type][extendsService] for the service under construction. */

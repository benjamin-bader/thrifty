 ...
 *   Default is `true`
 * @param namespaceResolver a lambda function to result namespaces for given [UserType]s. Default is
 *   to just use its Java namespace, but can be useful to configure it to look for alternate
 ...
 *   Normally when relativizing, paths are shortened to remove their combined common prefix. This
 *   can be specified to ensure that a minimumPrefix is kept for reference beyond the scope of this
 *   function. Example: `minPrefix = "foo/bar"` -> common prefix with `bar/baz` will be
 *   `foo/bar/baz`.
...
fun Schema.multiFileRender(
  ...
  // directory
  val commonPathPrefix =
      if (relativizeIncludes) {
        elements()
            .asSequence()
            .map(UserElement::filepath)
            .reduce { currentPrefix, nextLocation -> currentPrefix.commonPrefixWith(nextLocation) }
  ...
  return elements()
      ...
      .mapTo(LinkedHashSet()) { (filePath, sourceElements) ->
        ...
        val namespaces = elements.filterIsInstance<UserType>().map(UserType::namespaces)
        check(namespaces.distinct().size == 1) { "Multiple namespaces! $namespaces" }
        ...
        val fileSchema =
            toBuilder()
                ...
                .structs(
                    elements.filterIsInstance<StructType>().filter {
                      !it.isUnion && !it.isException
                    })
        ...
        val includes =
            elements
                .flatMap { element ->
                  when (element) {
                    is StructType -> {
                      element.fields.flatMap { it.type.unpack() }
                    }
                    is ServiceType -> {
                      element.methods.flatMap { method ->
                        (method.run { exceptions + parameters }).flatMap { it.type.unpack() } +
                ...
                .run {
                  if (relativizeIncludes) {
                    map {
                      it.first to
                          File(it.second).toRelativeString(sourceFile).removePrefix("../").run {
                            if (startsWith("../")) {
                              this
                            } else {
                              "./$this"
...
fun <A : Appendable> Schema.renderTo(buffer: A) =
    buffer.apply {
      if (typedefs.isNotEmpty()) {
        typedefs
            ...
            .joinEachTo(buffer = buffer, separator = DOUBLE_NEWLINE, postfix = DOUBLE_NEWLINE) {
                _,
                typedef ->
      ...
      if (enums.isNotEmpty()) {
        enums.sortedBy(EnumType::name).joinEachTo(
            buffer = buffer, separator = DOUBLE_NEWLINE, postfix = DOUBLE_NEWLINE) { _, enum ->
              enum.renderTo<A>(buffer)
            }
      }
      if (structs.isNotEmpty()) {
        structs.sortedBy(StructType::name).joinEachTo(
            buffer = buffer, separator = DOUBLE_NEWLINE, postfix = DOUBLE_NEWLINE) { _, struct ->
              struct.renderTo<A>(buffer)
            }
      }
      if (unions.isNotEmpty()) {
        unions.sortedBy(StructType::name).joinEachTo(
            buffer = buffer, separator = DOUBLE_NEWLINE, postfix = DOUBLE_NEWLINE) { _, struct ->
              struct.renderTo<A>(buffer)
            }
      }
      if (exceptions.isNotEmpty()) {
        exceptions.sortedBy(StructType::name).joinEachTo(
            buffer = buffer, separator = DOUBLE_NEWLINE, postfix = DOUBLE_NEWLINE) { _, struct ->
              struct.renderTo<A>(buffer)
            }
      }
      if (services.isNotEmpty()) {
        services.sortedBy(ServiceType::name).joinEachTo(
            buffer = buffer, separator = DOUBLE_NEWLINE, postfix = DOUBLE_NEWLINE) { _, service ->
...
private fun <A : Appendable> TypedefType.renderTo(buffer: A) =
    buffer.apply {
...
private fun <A : Appendable> StructType.renderTo(buffer: A) =
    buffer.apply {
      renderJavadocTo(buffer)
      val type =
          when {
      ...
      append(type, " ", name, " {")
      appendLine()
      fields.joinEachTo(buffer, NEWLINE) { _, field -> field.renderElementTo(buffer) }
      appendLine()
      append("}")
      renderAnnotationsTo(buffer)
    }

private fun <A : Appendable> EnumType.renderTo(buffer: A) =
    buffer.apply {
      renderJavadocTo(buffer)
      append("enum ", name, " {")
      appendLine()
      members.joinEachTo(buffer, ",$NEWLINE") { _, member -> member.renderElementTo(buffer) }
      appendLine()
      append("")
      append("}")
      renderAnnotationsTo(buffer)
...
private fun <A : Appendable> ServiceType.renderTo(buffer: A) =
    buffer.apply {
      renderJavadocTo(buffer)
      append("service ", name, " {")
      appendLine()
...
private fun <A : Appendable> Field.renderTo(buffer: A, indent: String = "  ") =
    buffer.apply {
    ...
    buffer.apply {
      ...
      if (parameters.isEmpty()) {
        ...
        parameters.joinEachTo(
            ...
            separator = ",$NEWLINE",
            prefix = "($NEWLINE",
            postfix = "$NEWLINE$indent)") { _, param ->
...
private fun <A : Appendable> EnumMember.renderTo(buffer: A, indent: String = "  ") =
    buffer.apply {
      renderJavadocTo(buffer, indent)
...
private fun <A : Appendable> Constant.renderTo(buffer: A) =
    buffer.apply {
      renderJavadocTo(buffer)
      append("const ")
      type.renderTypeTo(buffer, location)
...
private fun <A : Appendable> ConstValueElement.renderTo(buffer: A, prefix: String = " ") =
    buffer.apply { append(prefix, "= ", thriftText) }
...
private fun <A : Appendable> UserElement.renderJavadocTo(buffer: A, indent: String = "") =
    buffer.apply {
      if (hasJavadoc) {
        val docLines = documentation.trim().trim(Character::isSpaceChar).lines()
...
private fun <A : Appendable> UserElement.renderAnnotationsTo(
    ...
    indent: String = "",
    prefix: String = " "
) = buffer.apply { annotations.renderTo(buffer, indent, prefix) }
    ...
    buffer.apply {
      when {
        ...
        size > 1 -> {
          append(prefix)
          appendLine("(")
          entries.sortedBy(Map.Entry<String, String>::key).joinTo(
              buffer = buffer, separator = ",$NEWLINE") { (key, value) ->

...
/**
 * A set of callbacks that, when used with a [org.antlr.v4.runtime.tree.ParseTreeWalker], assemble a
 * [ThriftFileElement] from an [AntlrThriftParser].
 *
 * Instances of this class are single-use; after walking a parse tree, it will contain that parser's
 * state as thrifty-schema parser elements.
 *
 * @param tokenStream the same token stream used with the corresponding [AntlrThriftParser]; this
 *   stream will be queried for "hidden" tokens containing parsed doc comments.
...
internal class ThriftListener(
  ...
  fun buildFileElement(): ThriftFileElement {
    ...
    namespaces
        .find { it.scope == NamespaceScope.JVM }
        ?.let { jvmElement ->
  ...
  override fun exitStandardNamespace(ctx: AntlrThriftParser.StandardNamespaceContext) {
    ...
    val element =
        NamespaceElement(
            location = locationOf(ctx), scope = scope, namespace = name, annotations = annotations)
        ...
        if (ctx.requiredness() != null) {
          when {
            ...
            ctx.requiredness().text == "optional" -> Requiredness.OPTIONAL
                ...
                throw AssertionError("Unexpected requiredness value: " + ctx.requiredness().text)
          }
        } else {
          defaultRequiredness
        }
  ...
  private fun parseFunctionList(
      functionContexts: List<AntlrThriftParser.FunctionContext>
    ...
    for (ctx in functionContexts) {
      ...
      val returnType =
          if (ctx.fieldType() != null) {
            ...
            val token =
                ctx.getToken(AntlrThriftLexer.VOID, 0)
                    ?: throw AssertionError(
                        "Function has no return type, and no VOID token - grammar error")
      ...
      val maybeThrowsList = ctx.throwsList()?.let { parseFieldList(it.fieldList().field()) }
  ...
  private fun annotationsFromAntlr(
      ctx: AntlrThriftParser.AnnotationListContext?
  ): AnnotationElement? {
    if (ctx == null) {
      return null
    }
  ...
  private fun getLeadingComments(token: Token): List<Token> {
    ...
    return hiddenTokens?.filter { it.isComment && !trailingDocTokenIndexes.get(it.tokenIndex) }
        ?: emptyList()
  }
   ...
   *
   * INVARIANT: Assumes that the parse tree is being walked top-down, left to right!
   *
   * Trailing-doc tokens are marked as such, so that subsequent searches for "leading" doc don't
   * grab tokens already used as "trailing" doc. If the walk order is *not* top-down, left-to-right,
   * then the assumption underpinning the separation of leading and trailing comments is broken.
   *

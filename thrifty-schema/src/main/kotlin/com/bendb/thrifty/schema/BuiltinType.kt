...
/**
 * Represents types defined by the Thrift IDL, such as the numeric types, strings, binaries, and
 * void.
 */
class BuiltinType
internal constructor(name: String, override val annotations: Map<String, String> = emptyMap()) :
    ThriftType(name) {

  /** True if this represents a numeric type, otherwise false. */
  val isNumeric: Boolean
    get() = (this == I8 || this == I16 || this == I32 || this == I64 || this == DOUBLE)
  ...
  companion object {
    ...
    /** The (signed) byte type; identical to [BYTE], but with a regularized name. */
    ...
    /** A type representing a series of bytes of unspecified encoding, but we'll use UTF-8. */
    val STRING: ThriftType = BuiltinType("string")

    /** A type representing a series of bytes. */
    val BINARY: ThriftType = BuiltinType("binary")

    /** No type; used exclusively to indicate that a service method has no return value. */
    val VOID: ThriftType = BuiltinType("void")

    private val BUILTINS =
        listOf(BOOL, BYTE, I8, I16, I32, I64, DOUBLE, STRING, BINARY, VOID)
            .map { it.name to it }
            .toMap()

    /**
     * Returns the builtin type corresponding to the given [name], or null if no such type exists.
     */

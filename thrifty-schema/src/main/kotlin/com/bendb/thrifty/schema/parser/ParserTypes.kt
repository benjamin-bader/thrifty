 ...
 */
data class AnnotationElement(val location: Location, val values: Map<String, String>) {
 ...
 *
 * A map-type is a typical key-value lookup, a.k.a. associative array, dictionary, etc.
 *
 * @property location The location of the text corresponding to this element.
 ...
 */
data class IncludeElement(val location: Location, val isCpp: Boolean, val path: String)
 ...
 * @property value The literal value of the constant. This is not guaranteed by the parser to
...
data class StructElement(
  ...
  enum class Type {
    /**
     * A struct, in the C sense of the term. That is, an ordered set of named fields having
     * heterogeneous types.
     */
    STRUCT,

    /**
     * A union, also in the C sense of the term. A set of named fields, of which at most one may
     * have a value at the same time.
     */
    UNION,

    /**
     * An exception is like a [STRUCT], except that it serves as an error type that communicates
     * failure from an RPC call. Declared as part of a [FunctionElement].
     */
    EXCEPTION
  }
}
 ...
 *
 * A function defined with the `oneway` keyword has no return type, and generated clients will not
 * wait for a response from the remote endpoint when making a one-way RPC call. One-way function
 * must have a return type of `void` - this is not validated at parse time.
 *
 * @property location The location of the text corresponding to this element.
 ...
 *
 * A service is an entity having zero or more functions that can be invoked by remote clients.
 * Services can inherit the definition of other services, very much like inheritance in an
 * object-oriented language. The base type element here is presumed to identify another service, but
 * this is not validated at parse time.
 *
 * @property location The location of the text corresponding to this element.

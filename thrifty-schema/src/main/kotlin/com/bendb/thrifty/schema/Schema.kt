...
/**
 * Encapsulates all types, values, and services defined in a set of Thrift files.
 *
 * Strictly speaking, this is a lossy representation - the original filesystem structure of the
 * source Programs is not preserved. As such, this isn't a suitable representation from which to
 * generate C++ code, or any other module-based language for the matter. But as we're only concerned
 * with Java here, it's perfectly convenient.
 */
class Schema {
  ...
  internal constructor(
    ...
    fun structs(structs: List<StructType>): Builder = apply { this.structs = structs.toList() }

    /** Use the given [unions] for the schema under construction. */
    fun unions(unions: List<StructType>): Builder = apply { this.unions = unions.toList() }
    ...
    fun enums(enums: List<EnumType>): Builder = apply { this.enums = enums.toList() }
    ...
    fun typedefs(typedefs: List<TypedefType>): Builder = apply { this.typedefs = typedefs.toList() }

    /** Use the given [services] for the schema under construction. */
    fun services(services: List<ServiceType>): Builder = apply { this.services = services.toList() }
  ...
  /**
   * Returns a copy of the collection of [Constants][Constant] sorted in dependency order, suitable
   * for generating code in the order presented.
   *
   * Dependency-order is a reverse topological sort, and here means that for any given constant in
   * the list, all constants on which it depends (i.e. that it references) come before it in order.
   * For example:
   * ```
   * // given this thrift

...
class Loader {
  ...
  /**
   * A list of thrift files to be loaded. If empty, all .thrift files within [.includePaths] will be
   * loaded.
   */
  private val thriftFiles = mutableListOf<Path>()

  /**
   * The search path for imported thrift files. If [.thriftFiles] is empty, then all .thrift files
   * located on the search path will be loaded.
   */
  private val includePaths = ArrayDeque<Path>()

  /**
   * During include linking, we temporarily modify [includePaths] by prepending the including file's
   * directory. We don't want to consider these temporary modifications when evaluating an included
   * file's Location.
   */
  ...
  /**
   * Adds the given `path` to the set of directories from which included files will be located.
   *
   * @param path A [Path] identifying a directory, containing files to include.
   * @return this loader.
  ...
  /**
   * Parses all previously-given Thrift files, returning a [Schema] containing the parse results.
   *
   * If no Thrift files were specified, then all Thrift files on the include path are parsed. If the
   * include path is also empty, an exception is thrown.
   *
   * @return a [Schema] containing the parsed structs, consts, etc from the specified Thrift files.
   * @throws LoadFailedException if parsing fails for any reason.
   */
  ...
  private fun loadFromDisk() {
    ...
    if (filesToLoad.isEmpty()) {
      throw IllegalStateException(
          "No files and no include paths containing Thrift files were provided")
    }
    ...
    for (fileElement in loadedFiles.values) {
      ...
      if (!Files.exists(file)) {
        throw AssertionError("We have a parsed ThriftFileElement with a non-existing location")
      }
      if (!file.isAbsolute) {
  ...
  private fun loadFileRecursively(
    ...
    if (file != null) {
      ...
      element =
          loadSingleFile(dir, dir.relativize(file))
              ?: run {
                val suffix =
  ...
  internal fun resolveIncludedProgram(currentLocation: Location, importPath: String): Program {
    ...
    val resolved =
        findFirstExisting(importPathPath, currentLocation.asPath.parent)
  ...
  private fun findFirstExisting(path: Path, currentLocation: Path?): Path? {
    ...
    val firstExisting =
        includePaths.map { it.resolve(path).normalize() }.firstOrNull { Files.exists(it) }

...
/**
 * A test of auto-generated service code for the standard ThriftTest service.
 *
 * Conformance is checked by roundtripping requests to a local server that is run on the official
 * Apache Thrift Java codebase. The test server has an implementation of ThriftTest methods with
 * semantics as described in the .thrift file itself and in the Apache Thrift git repo, along with
 * Java code generated by their compiler.
 */
abstract class CoroutineConformanceTests {
  companion object {
     ...
     *
     * The server's transport and protocols are configured based on values returned by the abstract
     * methods [.getServerProtocol] and [.getServerTransport].
     */
    @RegisterExtension @JvmField val testServer: TestServer = TestServer()
    ...
    fun beforeAll() =
        runTest(timeout = 10.seconds) {
          ...
          val transport = SocketTransport.Builder("localhost", port).readTimeout(2.seconds).build()

          transport.connect()

          this@Companion.transport = decorateTransport(transport)
          this@Companion.protocol = createProtocol(this@Companion.transport)
          this@Companion.client = ThriftTestClient(protocol)
        }

    /**
     * When overridden in a derived class, wraps the given transport in a decorator, e.g. a framed
     * transport.
     */
  ...
  @Test fun testVoid() = runTest(timeout = 10.seconds) { client.testVoid() shouldBe Unit }

  @Test fun testBool() = runTest(timeout = 10.seconds) { client.testBool(true) shouldBe true }

  @Test
  fun testByte() =
      runTest(timeout = 10.seconds) { client.testByte(200.toByte()) shouldBe 200.toByte() }

  @Test fun testI32() = runTest(timeout = 10.seconds) { client.testI32(404) shouldBe 404 }

  @Test
  fun testI64() =
      runTest(timeout = 10.seconds) { client.testI64(Long.MAX_VALUE) shouldBe Long.MAX_VALUE }

  @Test
  fun testDouble() = runTest(timeout = 10.seconds) { client.testDouble(Math.PI) shouldBe Math.PI }
  ...
  fun testNest() =
      runTest(timeout = 10.seconds) {
        ...
        val nest = Xtruct2(byte_thing = 4.toByte(), i32_thing = 5, struct_thing = xtruct)
  ...
  fun testStringMap() =
      runTest(timeout = 10.seconds) {
        val argument = mapOf("foo\no" to "bar", "baz" to "qu\rux", "one" to "more")
  ...
  @Test
  fun testTypedef() =
      runTest(timeout = 10.seconds) { client.testTypedef(Long.MIN_VALUE) shouldBe Long.MIN_VALUE }

  @Test
  fun testMapMap() =
      runTest(timeout = 10.seconds) {
        client.testMapMap(Integer.MAX_VALUE) shouldBe
            mapOf(
                -4 to mapOf(-4 to -4, -3 to -3, -2 to -2, -1 to -1),
                4 to mapOf(1 to 1, 2 to 2, 3 to 3, 4 to 4))
      }
  ...
  @Test
  fun testExceptionInternalError() =
      runTest(timeout = 10.seconds) {
        try {
  ...
  @Test
  fun testMultiExceptionErrorOne() =
      runTest(timeout = 10.seconds) {
  ...
  @Test
  fun testMultiExceptionErrorTwo() =
      runTest(timeout = 10.seconds) {
        try {
  ...
  @Test fun oneway() = runTest(timeout = 10.seconds) { client.testOneway(secondsToSleep = 0) }
}
